...
```{r}
prices_with_countries <- names(prices) <- countries
prices_with_countries[c(1:5)]
```
...
```{r}
prices_with_countries['Canada']
prices_with_countries['United States']
prices_with_countries['Mexico']
```

6. Use inline code and the function `length()` to display the following sentence:
```{r}
{r length("The are x observations in the Big_Mac Index")}
```
