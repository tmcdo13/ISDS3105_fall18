...
```
`r {mean(as.numeric(iphoneSales[iphoneSales$time == 'Jun 2018',]),na.rm = TRUE)}`
...
#iphoneSales_tibble[iphoneSales_tibble$year == '2017']
...
You can "pipe" the above functions using `%>%` (shortcut: shift+cmd/ctrl+M), which passes the output that preceeds `%>%` to the first argument of the subsequent function (or alternatively anywhere you place a `.`). For instance:
