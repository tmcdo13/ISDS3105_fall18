...
Note how "filling" `t` is more efficient than adding an element to `t` at each iteration:
...
Assignment 4 asked to plot a map of insurance premiums of the Southern States only. Below is an approach to rethink at that problem in case we had to plot multiple regions on different maps. Since each map will show the same variables, we can think about iterating the call to `ggplot()`:
