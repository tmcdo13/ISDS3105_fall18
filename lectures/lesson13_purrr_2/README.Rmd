...
if(!dir.exists(here("ISDS3105/lectures/lesson13_purrr_2/timeLines")))
  dir.create(here("ISDS3105/lectures/lesson13_purrr_2/timeLines"))
drawMap <- function(hotels, title) {
                   ggplot(data = hotels) +
                      geom_polygon(aes(x = long, y = lat, group = group, fill = insurance_premiums), color = "white") + 
                      coord_fixed(1.3) +
                      labs(title = paste('Insurance Preiums for ', title)) +
    scale_fill_gradient(labels = scales::dollar_format())
  + ggsave(paste(title,".pdf"), path = "ISDS3105/lectures/lesson13_purrr_2/timeLines")
}
```
